...
def login():
    ...
    if form.validate_on_submit():
        ...
        if form.user.totp_enabled:
            ...
            return flask.redirect(flask.url_for("two_factor", next=next_page))
        else:
            flask_security.utils.login_user(form.user, remember=remember)
            models.db.session.commit()
            url = flask_security.utils.get_post_login_redirect(next_page)
...
def two_factor():
    ...
    if form.validate_on_submit():
        ...
        models.db.session.commit()
        url = flask_security.utils.get_post_login_redirect(form.next.data)
        return flask.redirect(url)
...
import views.manage  # noqa
import views.security  # noqa: E402
import views.signup  # noqa: E402
